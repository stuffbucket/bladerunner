...
func TestEveryCommandDeclaresAnInstancePolicy(t *testing.T) {
	want := map[string]string{
		...
		// `br ssh` is an alias of shell, so it does not appear here as its own
		// command; ssh-config is the verb that used to be called ssh.
		"br ssh-config":  instanceHonored,
